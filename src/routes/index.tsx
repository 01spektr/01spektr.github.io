...
function HomeToolCard({ tool, popular, fresh }: { tool: ToolDef; popular?: boolean; fresh?: boolean }) { const { locale } = useI18n(); const Icon = iconByName(tool.icon); const category = CATEGORIES.find((item) => item.id === tool.categoryId); return <Link to="/tools/$slug" params={{ slug: tool.slug }} className="home-tool-card"><span className="home-tool-icon" style={{ color: category?.tint, backgroundColor: `${category?.tint ?? "#3b82f6"}18` }}><Icon className="size-5" /></span>{(popular || fresh) && <em>{fresh ? "NEW" : "Популярное"}</em>}<b>{tool.name[locale]}</b><p>{tool.description[locale]}</p><ArrowRight className="home-tool-arrow size-4" /></Link>; }
